...
3. **Understand you can reshape long-to-wide and wide-to-long using `dcast()` and `melt()` function.**
...
3. Calculate the median, interquartile range ($IQR = Q3 − Q1$) for `arr_delays` of flights in in the `msp_feb_flights` data and the number of flights, grouped by `carrier`. Which carrier has the most variable arrival delays?
