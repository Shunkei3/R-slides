...
format: 
  revealjs:
    ...
    preview-links: true
webr:
  packages: ["data.table", "tidyverse", "rio", "nycflights13", "AER"]
  cell-options:
...
For example, let's subset rows where carrier is "AA" (American Airlines)
```{webr-r}
flights[carrier == "AA",]
```
...
### Basics
+ [`data.table` syntax: `DT[i, j, by]`]{style="color: blue;"}
+ [`j` argument not only allows you to select columns but also to compute on columns]{style="color: blue;"}
...
In the code above, three calculations are being performed in the `j` argument:
...
+ [`data.table` syntax: `DT[i, j, by]`]{style="color: blue;"}
+ [Using `i` and `j` argument together, you can perform calculation on columns of the subsetted rows.]{style="color: blue;"}
...
# the problem in the code above is that newly defined variable `total_delay` is referred in the same [] expression.
...
### Quiz
1. Let's create a new column `speed` by dividing `distance` by `air_time`.
...
If you don't want to do this intermediate step every time, you can do it like this, but it's hard to read!
...
[`%>%` takes the output of the code on its left and feed it as the first argument to the function on its right.]{style="color: blue;"}
...
::: {.callout-tip}
+ Always use `.` in the subsequent function to explicitly denote the destination of the object defined before `%>%` even if it is the first argument.
:::
...
```{webr-r}
#| auto-run: true
flights <- nycflights13::flights #load flights dates
...
Specifically, you learned the following tasks: Subset rows, Select columns, Compute on columns, Create a new column, Perform aggregations by group


1. **Understand you can reshape long-to-wide and wide-to-long using `dcast()` and `melt()` functions.**

2. **Understand you can merge data with `merge()` function.**
:::
...
For this exercise problem, we will use `journal` data from `AER` package. First, load the data and convert it to data.table object using `setDT` function (or. `as.data.table()`). Take a look at the data. Also, type `?journal` to see the description of the data.
