...
[You can change the theme of the plot.]{style="color: blue;"}
...
`theme()` function let you tweak the details of all non-data related components of a plot (e.g., font type in the plot, position of the legend and title, etc.). There are so many components you can modify with `theme()` function. See [this](https://ggplot2.tidyverse.org/reference/theme.html) for full list of options.
...
<!-- start pane-tabset: Excercise Problem 2 -->
:::{.panel-tabset}
...
ggplot(data = economics_long) +
  ...
  facet_wrap(vars(variable), scales = "free_y")
```
<!-- end pane-tabset: Excercise Problem 2 -->
:::
...
6. Create a new dataset containg the overall average corn yields across states by taking the mean of the `yield` by `year`. Add a line plot of this dataset to the plot you created in the previous step. Use red dashed line to represent this line.
