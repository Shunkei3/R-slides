...
2. [Introduction to ggplot2]{#introduction-to-ggplot2}
...
Install the package `ggplot2` if you haven't already done so.
```{r}
#| eval: false
...
##  Introduction to ggplot 2 Package {#intro}
...
<!-- ::: aside
Reference: [ggplot2: Elegant Graphics for Data Analysis (3e)](https://ggplot2-book.org/)
::: -->
...
+ For example, we use `geom_point()` to create a scatter plot. 
  + [use `aes()` to specify which variable you want to use for x and y axis]{style="color:red"}.
...
:::{.fragment .fade-up}
::: {.callout-tip}
+ When you want to make a simple x and y plot, the base R functions are sufficient (e.g., `with(data, plot(column_x, column_y)`). I do this often. 
:::
:::
...
[We can modify how plots look by specifying color, shape, and size]{style="color: blue;"}.
...
So far, we used only one `geom_*()` function in a plot. [But you can use multiple `geom_*()` functions in a single plot]{style="color:blue;"}.
...
# Of course, you can also use the import function from the rio package (this only works in RStudio, not here). For example, 
