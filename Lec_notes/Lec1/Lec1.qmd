...
2. [Basic data types (i.e., atomic data types) in R](#basic-data)
3. [Types of Data Structures in R](#data-str)
   1. [Vector (one-dimensional array)](#vec)
   2. [Matrix (Two-dimensional array)](#mat)
   3. [Data Frame](#df)
   4. [List](#list)
...
# Basic data types (i.e., atomic data types) in R {#basic-data}
...
+ [If you use character type, you alway need to put the text in either single quotes (') or double quotes (").]{style="color:red;"}
:::
...
+ As we will see later, [a sequence of logical values (i.e., logical vectors) can be used as an index vector to subset the data.]{style="color:red;"}
...
+ [You can use a logical vector as an index vector to subset elements of the vector. Only the elements that correspond to TRUE values are returned]{style="color:red;"}.
...
# extract the element in the Nitrogen and Yield column
...
[With these functions, you can avoid typing the data frame name and `$` mark, repeatedly.]{style="color:red;"}
...
2. If you are working with a team, your code might not work for other person because the path to the data file is different for each people.
:::
...
**Reasons**
+ If you work with R, there is no reason to save the data in other formats than `.rds`.
...
# Exercise Problems {#end-exercise}
...
Let's use `flight.rds` data in the `Data` folder. Load the data. We will only look at the flight data with "MSP" (Minneapolis–Saint Paul International Airport) as the destination airport. 
...
Here are the steps that I took to solve the problem. You don't need to follow the steps.
...
4. Count the binary category `delay_type` by `carrier`. Let's name the result `flights_mn_delay_count`.
   + `table()` is an R base function to create a frequency table. For example, `table(data$x)` creates a frequency table of `x` in the data. `table(data$x, data$y)` creates a frequency table of each combination of `x` and `y` in the data.
