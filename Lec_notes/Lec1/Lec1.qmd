...
### **Learning Objectives**
+ To get familiar with the features of RStudio.
...
+ To be able to use base R functions to do mathematical calculations.
+ To be able to use base `plot()` function for quick visualize data.
:::
...
+ How to open files or folder in Rstudio?
...
+ You can assign information (numbers, character, data) to an object on R using `<-` or `=` (e.g., `object_name <- value`) and reuse it later.
  + e.g `x <- 1` (globally) assigns 1 to an object called `x`.
    + In later, when we create functions, we will see that the object created in the function is only available in the function (not globally available).
...
+ As we will see later, a sequence of logical values (i.e., logical vectors) can be used index vector to subset the data!
:::
...
+ A vectors is a class of object that consists of elements of the **same kind of elements** (All elements in a vector must be of the same data type). You use `c()` to create a vector.
  + As other functions to create vector, you can use `seq()`, `rep()`.
...
+ You can get a single element, or a set of specific elements of a vector by indexing with `[]` brackets. Inside `[]`, you provide another vector (i.e. index vector) the position of the element you want to extract 
...
:::{.notes}
+ While vector is one-dimensional array, matrix is two-dimensional array.
+ 
:::
...
Don't worry about the details. You'll get used to these data structures as you use R more. Here are the key points you need to know at this point:
...
+ I am not sure if this works on your machine (especially for Windows). But you can try it if you're interested in speeding up R.
