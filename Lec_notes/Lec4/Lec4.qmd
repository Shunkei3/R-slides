...
datasummary(
  formula = 
    year * earnings ~ gender * (Mean + SD + Min + Max),
  data = CPSSW3,
  output = "gt"
  )
```
...
datasummary(
  formula = 
    (`Part time job` = parttime) * ((`Wage (dollars/week)` = wage) + (`Education (years)` = education) + (`Work experience (years)` = experience)) ~ ethnicity * (Mean + (`Std.Dev` = SD)),
  data = CPS1988,
  output = "gt"
  )
```
